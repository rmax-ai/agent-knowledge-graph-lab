...
import { LadybugGraphStore } from "../ladybug-graph-store.js";
...
function createMemoryFactory(): StoreFactory {
  return {
    name: "MemoryGraphStore",
    create: () => new MemoryGraphStore(),
  };
}

function createLadybugFactory(): StoreFactory {
  const dir = mkdtempSync(join(tmpdir(), "agkl-ladybug-contract-"));
  const databasePath = join(dir, "graph.lbug");
  return {
    name: "LadybugGraphStore",
    create: () => new LadybugGraphStore({ databasePath }),
    cleanup: () => {
      try {
...
const factories: StoreFactory[] = [createMemoryFactory(), createLadybugFactory()];

for (const factory of factories) {
  describe(`GraphStore contract (${factory.name})`, () => {
